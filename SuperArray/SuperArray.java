...
public class SuperArray									{
	...
	public boolean add(String e)						{
		if (dullArray.length == size)					{
			String[] tmpArray = 
				new String[dullArray.length]			;
			for (int i=0; i<dullArray.length; i++)		{
				tmpArray[i] = dullArray[i]				;}
			dullArray =
				new String[dullArray.length*2]			;
	...
	public String remove(int index)						{
		...
		for (int i=0; i<dullArray.length; i++)			{
			dullArray[i] = tmpArray[i]					;}
		dullArray[size-1] = null						;
			// save memory, make old value null
		size--											;
		return returnString								;}
